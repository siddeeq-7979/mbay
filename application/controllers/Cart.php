...
//Action based on payment method
    ...
    public function add_to_cart() {
        ...
        if ($get['product_id']) {
            ...
            if ($product_details) {
                ...
                //     $cart[$key]['option_value'] = $this->shop_model->optionValueName($cart_item['options']);
                // }
                ...
                $data = array(
                    ...
                    'price' => $product_details['product_amount'] + $option_value_price,
                ...
                if ($res) {
                    echo json_encode([ "status"=>"yes","data"=> count($this->cart->contents())]);
                    exit();
                }
            } else {
                echo json_encode([ "status"=>"no","data"=>  lang('product_quantity_exceeded_stock')]);
                exit();
            }
        }
        echo  json_encode([ "status"=>"no","data"=>  lang('failed_to_add_js')]);
        exit();
    }
    /**
     * For add the product/package where added to the cart
     ...
     * @copyright (c) 2017, Techffodils Technologies
     */
    public function add_option_to_cart() {
        ...
        if($this->session->userdata('product_option_data')){
            ...
            if($res_da){
                ...
                foreach ($res_da as $res_val) {
                    $data[$product_id][$res_val['option_id']] = $res_val['id'];
                }
            }
         }

        $data[$product_id][$option_id] = $option_value_id;
        $this->session->set_userdata('product_option_data', $data);
        ...
        $price = 0;
        $this->load->model('product_model');
        $product_data = $this->product_model->getProductDetails($product_id);
        $price +=  $product_data['product_amount'];

        foreach ($data[$product_id] as $option_value) {
           $op_data =  $this->product_model->getProOptions($option_value);
           if($op_data['price_change'] == '-'){
            $price -= $op_data['price'];
           }else{
    ...
    function defaultOptionData($pro_id) { 
        ...
        $query = $this->db->select('option_id,id')
                ->where('pro_id', $pro_id)
