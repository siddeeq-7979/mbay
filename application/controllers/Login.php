...
class Login extends Base_Controller {
    ...
    function site_login() {
        ...
        if ($run) {
            ...
            // print_r($post_data['username']);die;
    ...
    function validate_affiliate_username_email($employee_username = '') {
     ...
     function validate_exist_username($username = '') {
        if ($username != '') {
            $flag = false;
            if ($this->helper_model->userNameToID($username)==0) {
                $flag = TRUE;
            }
            return $flag;
        } elseif ($this->input->get('username')) {
            if ($this->helper_model->userNameToID($this->input->get('username'))==0) {
                echo 'yes';
                exit();
            }
            echo 'no';
