...
class Product extends Base_Controller {
    ...
    public function product_options($action = "", $pro_id = "", $val_id = "") {
        ...
        if($this->input->post('add_pro_opt')){
            ...
            $value_post = $this->security->xss_clean($this->input->post());
            // print_r($value_post);die;

            $option_id = $value_post['option_ids'];
    ...
    function get_option_values($id = 'option_value') {
        $get = $this->input->get();
        $tab_index = isset($get['tab_index']) ? $get['tab_index'] : '0';
        $options = "<select class='form-control search-select' id='" . $id . "' name='" . $id . "' tabindex='" . $tab_index . "'>";
        $val= $this->input->get('option_id');
        ...
        $options .= "<option value='" . $opt['id'] . "' style='background-color:".$opt['option_value']."!important;color:".$opt['option_value']."!important;'>
        </option>";
             }
        }else{
         $option_values = $this->product_model->getOptionValueLists($this->input->get('option_id'));
    ...
    function get_pro_option_value_details(){
        $id=$this->input->get('opt_id');

        $product_options_details = $this->product_model->getProOptionsDetails($id);
    
       

       echo json_encode($product_options_details);
       exit();
    }
    ...
    function change_default_value() {
        ...
        $option=$this->product_model->getOptionValue($id);
        $product_options = $this->product_model->updateDefaultValue($id,$pro_id,$option);
        echo json_encode($product_options);
        exit();
    }
}
