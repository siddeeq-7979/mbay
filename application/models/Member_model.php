...
class Member_model extends CI_Model {
    ...
    function getInvoiceDetails($ord_id) {
        $ord = array();
        $query = $this->db->select('user_id,name,phone,total_amount,delivery_charge,shipping_charge,tax,total_pv,payment_type,address1,address2,country,state,city,zip_code,confirm_date,discount')
                ->where('id', $ord_id)
                ->from('orders')
                ->get();
        if ($query->num_rows() > 0) {
            ...
            $ord['username'] = $query->row()->name;
            $ord['phone'] = $query->row()->phone;
            $ord['address1'] = $query->row()->address1;
            $ord['address2'] = $query->row()->address2;
            ...
            $ord['discount'] = $query->row()->discount;
    ...
    function getOrderProducts($order_id) {
        $data = array();
        $query = $this->db->select("id,amount,quantity,product_id,date,option_value")
                ->from("order_products")
                ->where("order_id", $order_id)
                ->get();
        $i = 0;
    ...
    function getAllOrdersData(){        
        $data = array();
        $query = $this->db->select('orders.id, orders.order_status,orders.total_amount,orders.order_date,user_name,orders.payment_type,orders.type,orders.images')
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result_array() as $row) {
                ...
                $data[$i]['images'] = unserialize($row['images']);
                if($data[$i]['images']){
                $data[$i]['file_name'] = $data[$i]['images']['file_name'];
                }
                $i++;
            }
        }
    ...
    return $status_name;
     ...
     function getPaymentSlip($ord_id) {
        $ord = array();
