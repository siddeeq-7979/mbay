...
if ($where) {
    ...
    public function getUserAddresses($user_id){
        $detail = $this->db->select('id, address_1 ,address_2, city, country_id, state_id,zip_code')
        ->from('user_address')
    ...
    public function updateAddaddress($address_post,$user_id=''){
            ...
            // if($default=='on'){ 
                ...
                if(array_key_exists("address",$address_post)){
                    foreach($address_post['address'] as $updateaddress){
                        ...
                        ->set('status',  $updateaddress['default']??0)
                        ->insert('user_address');  
                    }
                }

                if(array_key_exists("ads",$address_post)){

                    foreach($address_post['ads'] as $updateaddres){
                        $id = $updateaddres['id'];

                        $data = [
                            'address_1' => $updateaddres['address_1'],
                            'address_2' => $updateaddres['address_2'],
                            'city' => $updateaddres['city'],
                            'country_id' => $updateaddres['country_id'],
                            'state_id' => $updateaddres['state'],
                            'zip_code' =>$updateaddres['zip_code'],
                            'status' =>$updateaddres['default']??0,
                        ];
                        $this->db->where('id', $id);
    ...
    public function joinuserdetails(){
       ...
       $datauser = array();
        foreach ($query->result() as $row) {
                   ...
                   $datauser[$i]['date'] = date('Y-m-d',strtotime($row->date));
